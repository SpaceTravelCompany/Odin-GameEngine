...
@(private = "file") g_sounds:map[^sound]struct{}
@(private = "file") sound_allocator:runtime.Allocator
...
@(private = "file") callback :: proc() {
    for intrinsics.atomic_load_explicit(&started, .Relaxed) {
        ...
        if this != nil {
            ...
            if !miniaudio.sound_is_looping(&this.__miniaudio_sound) {
                deinit2(this)
            }
        }
    }
...
@(private = "file") deinit2 :: proc(self:^sound) {
    if !self.__inited do return
    miniaudio.sound_uninit(&self.__miniaudio_sound)
    miniaudio.audio_buffer_uninit(&self.__miniaudio_audioBuf)
    if intrinsics.atomic_load_explicit(&started, .Relaxed) do delete_key(&g_sounds, self)
	for sound, i in self.src.sounds {
		if sound == self {
			unordered_remove(&self.src.sounds, i)
			break
		}
	}
	free(self, sound_allocator)
}
...
@(fini, private) g_deinit :: proc "contextless" () {
    if !intrinsics.atomic_load_explicit(&started, .Relaxed) do return
    intrinsics.atomic_store_explicit(&started, false, .Relaxed)
    sync.sema_post(&g_sema)
...
sound_src_play_sound_memory :: proc(self:^sound_src, volume:f32, loop:bool) -> (snd: ^sound, err: sound_error) {
    if !intrinsics.atomic_load_explicit(&started, .Relaxed) do log.panicf("sound_src_play_sound_memory : sound not started.\n")

    err = .SUCCESS
    snd = new(sound, sound_allocator)
    defer if err != .SUCCESS do free(snd, runtime.default_allocator())
    snd^ = sound{ src = self }
    ...
    err = miniaudio.sound_init_from_data_source(
        ...
        flags = {.DECODE},
        pGroup = nil,
        pSound = &snd.__miniaudio_sound,
    )
    if err != .SUCCESS do return
    miniaudio.sound_set_end_callback(&snd.__miniaudio_sound, end_callback, auto_cast snd)
    miniaudio.sound_set_looping(&snd.__miniaudio_sound, auto_cast loop)

    miniaudio.sound_set_volume(&snd.__miniaudio_sound, volume)

    err = miniaudio.sound_start(&snd.__miniaudio_sound)
    if err != .SUCCESS {
        miniaudio.sound_uninit(&snd.__miniaudio_sound)
        return
    }

    sync.mutex_lock(&g_sounds_mtx)
    map_insert(&g_sounds, snd, struct{}{})
    append(&self.sounds, snd)
    sync.mutex_unlock(&g_sounds_mtx)
...
set_volume :: #force_inline proc "contextless" (self:^sound, volume:f32) {
    miniaudio.sound_set_volume(&self.__miniaudio_sound, volume)
}

//= playing speed
set_pitch :: #force_inline proc "contextless" (self:^sound, pitch:f32) {
    miniaudio.sound_set_pitch(&self.__miniaudio_sound, pitch)
}

pause :: #force_inline proc (self:^sound) {
    res := miniaudio.sound_stop(&self.__miniaudio_sound)
    if res != .SUCCESS {
...
resume :: #force_inline proc (self:^sound) {
   res := miniaudio.sound_start(&self.__miniaudio_sound)
   if res != .SUCCESS {
...
@require_results get_len_sec :: #force_inline proc (self:^sound) -> f32 {
    sec:f32
    res := miniaudio.sound_get_length_in_seconds(&self.__miniaudio_sound, &sec)
    if res != .SUCCESS {
...
@require_results get_len :: #force_inline proc (self:^sound) -> u64 {
    frames:u64
    res := miniaudio.sound_get_length_in_pcm_frames(&self.__miniaudio_sound, &frames)
    if res != .SUCCESS {
...
@require_results get_pos_sec :: #force_inline proc (self:^sound) -> f32 {
    sec:f32
    res := miniaudio.sound_get_cursor_in_seconds(&self.__miniaudio_sound, &sec)
    if res != .SUCCESS {
...
@require_results get_pos :: #force_inline proc (self:^sound) -> u64 {
    frames:u64
    res := miniaudio.sound_get_cursor_in_pcm_frames(&self.__miniaudio_sound, &frames)
    if res != .SUCCESS {
...
set_pos :: #force_inline proc (self:^sound, pos:u64) {
    res := miniaudio.sound_seek_to_pcm_frame(&self.__miniaudio_sound, pos)
    if res != .SUCCESS {
...
set_looping :: #force_inline proc "contextless" (self:^sound, loop:bool) {
    miniaudio.sound_set_looping(&self.__miniaudio_sound, auto_cast loop)
}

@require_results is_looping :: #force_inline proc "contextless" (self:^sound) -> bool {
   return auto_cast miniaudio.sound_is_looping(&self.__miniaudio_sound)
}

@require_results is_playing :: #force_inline proc "contextless" (self:^sound) -> bool {
    return auto_cast miniaudio.sound_is_playing(&self.__miniaudio_sound)
}

@require_results sound_src_decode_sound_memory :: proc(data:[]byte, allocator := context.allocator) -> (result : ^sound_src, err: sound_error) {
    if !intrinsics.atomic_load_explicit(&started, .Relaxed) do g_init()//?g_init 따로 호출하지 않고 최초로 사용할때 시작

    result = new(sound_src, allocator)
    defer if err != .SUCCESS do free(result, allocator)
    ...
    result.sounds = make([dynamic]^sound, allocator)
    return
}
