...
@(require_results, private) __base_mat :: #force_inline proc "contextless" (self_component:^gui_component, mul: linalg.point) -> Maybe(linalg.matrix44) {
    return linalg.sr_2d_matrix2(self_component.gui_scale, self_component.gui_rotation, self_component.gui_center_pt * mul)
}
    ...
    switch self_component.gui_align_x {
        case .left:
            switch self_component.gui_align_y {
                case .top:
                    ...
                    mat = linalg.t_2d_matrix(linalg.point3d{-window_width / 2.0 + self_component.gui_pos.x, window_height / 2.0 - self_component.gui_pos.y, 0.0})
                case .middle:
                    base = __base_mat(self_component, linalg.point{1.0, 1.0})
                    mat = linalg.t_2d_matrix(linalg.point3d{-window_width / 2.0 + self_component.gui_pos.x, self_component.gui_pos.y, 0.0})
                case .bottom:
                    base = __base_mat(self_component, linalg.point{1.0, 1.0})
                    mat = linalg.t_2d_matrix(linalg.point3d{-window_width / 2.0 + self_component.gui_pos.x, -window_height / 2.0 + self_component.gui_pos.y, 0.0})     
            }
        case .center:
            switch self_component.gui_align_y {
                case .top:
                    base = __base_mat(self_component, linalg.point{1.0, -1.0})
                    mat = linalg.t_2d_matrix(linalg.point3d{self_component.gui_pos.x, window_height / 2.0 - self_component.gui_pos.y, 0.0})
                case .middle:
                    base = __base_mat(self_component, linalg.point{1.0, 1.0})
                    mat = linalg.t_2d_matrix(linalg.point3d{self_component.gui_pos.x, self_component.gui_pos.y, 0.0})
                case .bottom:
                    base = __base_mat(self_component, linalg.point{1.0, 1.0})
                    mat = linalg.t_2d_matrix(linalg.point3d{self_component.gui_pos.x, -window_height / 2.0 + self_component.gui_pos.y, 0.0})     
            }
...
Inputs:
- self: Pointer to the object (must be a subtype of itransform_object)
...
gui_component_size ::  proc (self:^$T, self_component:^gui_component)
    ...
    engine.itransform_object_update_transform_matrix_raw(auto_cast self, self.mat)
}
