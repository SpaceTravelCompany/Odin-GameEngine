...
try_cross_linking:;
		...
		if (is_windows) {
			...
			//int const ODIN_ANDROID_API_LEVEL = build_context.ODIN_ANDROID_API_LEVEL; // edited (xfitgd)
					...
					if (has_asm_extension(lib)) {
						...
						if (build_context.metrics.os == TargetOs_darwin) {
							...
							if (string_ends_with(lib, str_lit(".a")) || string_ends_with(lib, str_lit(".o")) || string_ends_with(lib, str_lit(".so")) || string_contains_string(lib, str_lit(".so."))) {
								lib_str = gb_string_append_fmt(lib_str, " \"%.*s\" ", LIT(lib));//edited (xfitgd) -l: works in my linux but windows does not works.
							} else {
								// dynamic or static system lib, just link regularly searching system library paths
								lib_str = gb_string_append_fmt(lib_str, " -l%.*s ", LIT(lib));
							}
			...
			if (is_android) { // NOTE(bill): glue code needed for Android
				...
				glue = gb_string_append_fmt(glue, " --target=aarch64-linux-android%d ", build_context.ODIN_ANDROID_NDK_API_LEVEL);// edited (xfitgd)
			...
			if (is_android) {
				...
				link_command_line = gb_string_append_fmt(link_command_line, " --target=aarch64-linux-android%d ", build_context.ODIN_ANDROID_NDK_API_LEVEL);// edited (xfitgd)
			} else {
